...
`bin/backfill.ts` is the provider-neutral dispatcher for this historical path. It resolves initialized accounts from `shiplog.config.json`, refreshes the current GitHub login from the stable account ID, and invokes `bin/backfill_github.ts`. `BACKFILL_MODE`, `BACKFILL_MAX_MINUTES`, and `BACKFILL_REPO_BUDGET_MINUTES` can choose the fast or deep path and cap unfinished repository work in a run. `BACKFILL_REPOSITORY_LIMIT` remains available for custom local runs that need a hard repository cap. The bundled `history` workflow exposes only `max_minutes` and `repo_budget_minutes`, defaulting to a 30-minute run cap and a 5-minute budget before starting another repository. Repository-level success, retry-wait, completed-step state, and private candidate probe cursors let later runs resume the remaining work without letting one transient repository error or very large private repository fail the whole backfill. When a budgeted run finishes its first repository pass with incomplete private probes and still has enough runtime reserved, it revisits those probe cursors before finishing. Lost private or organization access is warning-only for the current run and is not written as permanent blocked state. An isolated primary-token rejection inside private-repository work is warning-only only after an immediate account check succeeds; a primary token that fails account validation remains fatal. `BACKFILL_REQUIRE_COMPLETE=true` is still available for local or custom workflows that need an incomplete paused run to exit unsuccessfully, but the bundled `history` workflow leaves it off so progressive history can continue improving the database between scheduled `publish` runs.
