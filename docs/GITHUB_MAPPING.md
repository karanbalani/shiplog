...
Backfill supports two modes. `fast` mode is the default first-run path. It uses contribution-group repositories and GitHub's author-filtered commit history query so large repositories do not require broad default-branch scans for unrelated commits. `deep` mode trades runtime for reconciliation: it uses unfiltered commit history so `Co-authored-by` credits can be found, enumerates private repositories from REST `GET /user/repos?visibility=private` for the default read token, resolves configured organization node IDs to current logins, and enumerates configured organization repositories from REST `GET /orgs/{organization}/repos?type=all` with organization-specific read tokens. Deep-mode private repositories discovered only through listing are treated as candidates and are written to `repositories` only after a matching commit, pull request, issue, or review is found.

`bin/backfill.ts` is the provider-neutral dispatcher for this historical path. It resolves initialized accounts from `shiplog.config.json`, refreshes the current GitHub login from the stable account ID, invokes `bin/backfill_github.ts`, and advances the account checkpoint after the historical run completes all discovered repositories. `BACKFILL_MODE`, `BACKFILL_REPOSITORY_LIMIT`, and `BACKFILL_MAX_MINUTES` can choose the fast or deep path and cap unfinished repository work in a run; repository-level success, skip, retry-wait, and completed-step state lets later runs resume the remaining work without letting one transient repository error fail the whole backfill. `BACKFILL_REQUIRE_COMPLETE=true` turns an incomplete paused run into a non-successful command exit, which the GitHub Actions workflow uses to avoid triggering render on partial historical data.
