...
- A `config` type describes `shiplog.config.json` or another user-authored config file.
...
- Keep `schemas/shiplog.config.schema.json` as the source of truth for `shiplog.config.json` validation.
...
- Config must not contain secrets; database URLs and provider tokens belong in secrets. GitHub Actions token values are exported from config-declared `tokenEnv` names with `bun run tokens:export`.
