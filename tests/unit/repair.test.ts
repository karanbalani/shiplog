...
import * as repair from '../../bin/repair.ts'
...
const originalRepairDate = process.env.REPAIR_DATE
const originalRepairFrom = process.env.REPAIR_FROM
const originalRepairTo = process.env.REPAIR_TO
...
beforeEach(() => {
  ...
  delete process.env.REPAIR_DATE
  delete process.env.REPAIR_FROM
  delete process.env.REPAIR_TO
...
afterEach(async () => {
  ...
  restoreEnv('REPAIR_DATE', originalRepairDate)
  restoreEnv('REPAIR_FROM', originalRepairFrom)
  restoreEnv('REPAIR_TO', originalRepairTo)
...
test('run repairs REPAIR_DATE without moving the checkpoint', async () => {
  await seedAccount({ lastSuccessfulCollectOn: '2026-05-01' })
  process.env.REPAIR_DATE = '2026-05-07'
  process.env.COLLECT_DATE = '2026-05-06'
...
test('run repairs an explicit range without moving the checkpoint', async () => {
  await seedAccount({ lastSuccessfulCollectOn: '2026-05-01' })
...
test('run requires a repair date or complete repair range', async () => {
  await seedAccount()
  ...
  ).rejects.toThrow(/REPAIR_DATE or REPAIR_FROM and REPAIR_TO/)

  process.env.REPAIR_FROM = '2026-05-05'
  ...
  ).rejects.toThrow(/REPAIR_FROM and REPAIR_TO/)
})
